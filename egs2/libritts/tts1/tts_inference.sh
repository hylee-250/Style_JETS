...
if ! "${skip_eval}"; then
    if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            echo "data_path_and_name_and_type : ${_data}/vctk,text,text"
            echo "data_path_and_name_and_type : ${_speech_data}/${_vctk},speech,${_type}"
            ...
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/tts_inference.JOB.log \
                ${python} -m espnet2.bin.tts_inference \
                    --ngpu "${_ngpu}" \
                    --data_path_and_name_and_type "${_data}/vctk,text,text" \
                    --data_path_and_name_and_type ${_speech_data}/${_vctk},speech,${_type} \
                    --key_file "${_logdir}"/keys.JOB.scp \
