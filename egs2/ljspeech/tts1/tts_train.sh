...
if ! "${skip_train}"; then
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        ...
        _opts=
        # train_config가 빈 문자열이 아니라면
        # 여기선 train_config = train_jets.yaml
        if [ -n "${train_config}" ]; then
        ...
        # teacher_dumpdir이 빈 문자열이 아니라면
        # fastspeech이면 필요함 -> Pre-trained Tacotron 2 모델에서 duration을 추출할 것으로 보임
        ...
        # tts_stats_dir = ${expdir}/tts_stats_raw_phe_tacotron_g2p_en_no_space
        _logdir="${tts_stats_dir}/logdir"
        mkdir -p "${_logdir}"

        # Get the minimum number among ${nj} and the number lines of input files
        # nj와 train dataset, valid dataset 중에 가장 작은 숫자를 _nj로 설정
        _nj=$(min "${nj}" "$(<${_train_dir}/${_scp} wc -l)" "$(<${_valid_dir}/${_scp} wc -l)")

        # key_file = tr_no_dev/wav.scp
        ...
        # key_file =dev/wav.scp
        key_file="${_valid_dir}/${_scp}"
        split_scps=""
        for n in $(seq "${_nj}"); do
            split_scps+=" ${_logdir}/valid.${n}.scp"
        done
        # split_scps= train.1.scp,....,train.32.scp, valid.1.scp,...,valid.32.scp
        ...
        # python -m espnet2.bin.gan_tts_train
    ...
    if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
        ...
        # -z 문자열의 길이가 0이면 참 -> teacher_dumpdir가 없으면 참
        # AR 모델과 fully E2E model은 case 1
        ...
        # stage 6에서 시작할 수 있도록 run.sh 파일을 생성
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            echo "data_path_and_name_and_type : ${_speech_data}/${_scp},speech,${_type}"
            # shellcheck disable=SC2046,SC2086
